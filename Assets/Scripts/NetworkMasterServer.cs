...
public class NetworkMasterServer : MonoBehaviour
{
	...
	public void InitializeServer()
	{
		...
		NetworkServer.RegisterHandler(MasterMsgTypes.GuessId, OnServerGuess);
	...
	void OnServerGuess(NetworkMessage netMsg)
	{
		var msg = netMsg.ReadMessage<MasterMsgTypes.GuessMessage>();
		string guess = msg.guess;
		Debug.Log("OnServerGuess " + guess);

		guesses[netMsg.conn.connectionId] = guess;
		if (guesses.Count < 2)
			return;
		bool b = true;
		string lastGuess = null;
		foreach (KeyValuePair<int, string> entry in guesses) {
			if (entry.Value == "")
				b = false;
			else if (lastGuess == null)
				lastGuess = entry.Value;
			else if (lastGuess != entry.Value)
				b = false;
			if (!b)
				return;
		}
	...
	void OnGUI()
	{
		...
		// foreach (var rooms in gameTypeRooms.Values)
		// {
		// 	GUI.Label(new Rect(0, y, 200, 20), "GameType:" + rooms.name);
		// 	y += 22;
		// 	foreach (var room in rooms.rooms.Values)
		// 	{
